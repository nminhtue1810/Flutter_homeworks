...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'Celcius - Fahrenheit Converter'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final fahController = TextEditingController();

  @override
  void initState(){
    super.initState();
    //celciusController.addListener(_printLatestValue);
  }
          ...
          child: Column(
              children: <Widget>[
                TextField(
                  ...
                  onChanged: (text) {
                    fahController.text = '${celcius2Fahrenheit(double.parse(text))}';
                  },
                  keyboardType: TextInputType.number,
                  decoration: new InputDecoration(
                    labelText: 'Enter the Celcius (°C)',
                    hintText: 'Celcius (°C)',
                ...
                TextField(
                  onChanged: (text) {
                    celciusController.text = '${fahrenheit2Celcius(double.parse(text))}';
                  },
                  controller: fahController,
                  keyboardType: TextInputType.number,

                  decoration: new InputDecoration(
                    labelText: 'Enter the Fahrenheit (°F)',
                      hintText: 'Fahrenheit (°F)',
                      focusedBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.all(Radius.circular(10.0)),
                        borderSide: BorderSide(color: Colors.grey),
                      ),
